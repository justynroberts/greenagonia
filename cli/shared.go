...
// Shared/multi-user environment management.
// Commands are routed directly from main() — no "shared" prefix.
...
func defaultSharedTfDir() string {
	candidates := []string{"terraform-shared"}
	if exe, err := os.Executable(); err == nil {
		dir := filepath.Dir(exe)
		candidates = append(candidates, filepath.Join(dir, "terraform-shared"))
	}
...
func cmdUserToken(dir string) {
	tok, err := promptPassword("PagerDuty user-level API token (hidden): ")
	if err != nil || tok == "" {
		die("no token entered")
	}
	s, err := loadSharedSecrets(dir)
...
func cmdSlackToken(dir string) {
	...
	fmt.Printf("  %s run 'greenagonia deploy' to create the Slack channels\n", green("✓"))
}
...
func cmdAdmin(dir string, args []string) {
	if len(args) < 1 {
		cmdAdminList(dir)
		return
	}
	...
	case "add":
		if len(args) < 4 {
			die("usage: greenagonia admin add <INITIALS> \"Full Name\" <email> [slack-email]")
		}
		...
		cmdAdminAdd(dir, ini, name, email, slack)
	case "remove":
		if len(args) < 2 {
			die("usage: greenagonia admin remove <INITIALS>")
		}
		cmdAdminRemove(dir, strings.ToUpper(args[1]))
	case "list", "":
		cmdAdminList(dir)
	default:
		die("usage: greenagonia admin [add|remove|list]")
	}
}

func cmdAdminAdd(dir, ini, name, email, slackEmail string) {
	...
	if ex, ok := cfg.Admins[ini]; ok && strings.EqualFold(ex.Email, email) {
		...
		fmt.Printf("  run 'greenagonia deploy' to apply\n")
		return
	}
...
func cmdAdminRemove(dir, ini string) {
	cfg, err := loadSharedConfig(dir)
	if err != nil {
		die("reading config: " + err.Error())
	}
...
func cmdAdminList(dir string) {
	...
	if len(cfg.Admins) == 0 {
		...
		fmt.Println("  greenagonia admin add JR \"Justyn Roberts\" justyn@example.com")
		return
	}
...
func cmdSiteURL(dir string, args []string) {
	var u string
	if len(args) > 0 {
		u = args[0]
	} else {
	...
	fmt.Println("  run 'greenagonia deploy' to apply, then 'greenagonia urls' to see updated links")
}
...
func cmdSetup(dir string) {
	...
	if s.PagerDutyToken != "" {
		fmt.Printf("  %s token already configured (run 'greenagonia token' to replace)\n\n", green("✓"))
	} else {
		cmdToken(dir)
		fmt.Println()
	}
	...
	if len(cfg.Admins) == 0 {
		...
		cmdAdminAdd(dir, ini, name, email, "")
	}

	fmt.Printf("\n  %s setup complete\n", green("✓"))
	fmt.Printf("  next: greenagonia deploy\n\n")
}
...
func cmdDeploy(dir string) {
	if _, err := os.Stat(sharedSecretsPath(dir)); os.IsNotExist(err) {
		die("no token configured — run 'greenagonia setup' first")
	}
...
func cmdURLs(dir string, args []string) {
	filter := ""
	if len(args) > 0 {
	...
	if len(routing) == 0 {
		die("no outputs yet — run 'greenagonia deploy'")
	}
...
func slackAPI(token, method, path string, body map[string]any) (map[string]any, error) {
	...
	reqURL := "https://slack.com/api/" + path
...
func cmdSlackChannels(dir string) {
	s, err := loadSharedSecrets(dir)
	if err != nil || s.SlackBotToken == "" {
		die("no Slack credentials — run 'greenagonia slack-token' first")
	}
	cfg, err := loadSharedConfig(dir)
	if err != nil {
		die("reading config: " + err.Error())
